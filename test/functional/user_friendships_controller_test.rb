...
class UserFriendshipsControllerTest < ActionController::TestCase
      ...
      context "successfully" do
        should "create two user friendships" do
          assert_difference 'UserFriendship.count', 2 do
            post :create, user_friendship: { friend_id: users(:genius).profile_name }
          end
        end
      end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:laura), friend: @friend)
        create(:pending_user_friendship, friend: users(:laura), user: @friend)
        sign_in users(:laura)
  ...
  context "#edit" do 
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:laura))
        sign_in users(:laura)
      ...
      should "assing to user_friendship" do
        assert assigns(:user_friendship)
      end

      should "assing to friendship" do
        assert assigns(:friend)
      end             
    end              
  end
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:laura))
        create(:accepted_user_friendship, friend: users(:laura), user: @friend)
